/**
 * @file lib.c
 * @brief Файл з реалізацією функції для роботи зі рядками.
...
size_t count_num_in_text(char text[])
{
...
void del_dublicates(size_t count_of_words, char *words[count_of_words]) {
...
void calclulate_frequency(size_t count_of_symbols, char char_array[], char frequency_table_chars[], float *frequency_table_float) {
    ...
    calculate_count_of_symbols(count_of_symbols, char_array, count_of_symbols_in_text);
